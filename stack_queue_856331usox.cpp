...
int main() {
    ds_856331usox::Stack<int> stk;
    for(int x:{ 80, 43, 44, 74, 1, 98, 95, 89, 33, 84 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_856331usox::CQueue<int> q(20);
    for(int x:{ 80, 43, 44, 74, 1, 98, 95, 89, 33, 84 }) q.enqueue(x);
