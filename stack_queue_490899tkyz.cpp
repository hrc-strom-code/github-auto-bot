...
int main() {
    ds_490899tkyz::Stack<int> stk;
    for(int x:{ 63, 96, 10, 71, 92, 61, 9, 2 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_490899tkyz::CQueue<int> q(20);
    for(int x:{ 63, 96, 10, 71, 92, 61, 9, 2 }) q.enqueue(x);
