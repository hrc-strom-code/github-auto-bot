...
int main() {
    ds_857624yzni::Stack<int> stk;
    for(int x:{ 49, 56, 52, 91, 60, 94, 1, 5, 22, 65 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_857624yzni::CQueue<int> q(20);
    for(int x:{ 49, 56, 52, 91, 60, 94, 1, 5, 22, 65 }) q.enqueue(x);
