...
int main() {
    ds_585157qcbb::Stack<int> stk;
    for(int x:{ 48, 71, 60, 14, 72, 23, 78, 9 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_585157qcbb::CQueue<int> q(20);
    for(int x:{ 48, 71, 60, 14, 72, 23, 78, 9 }) q.enqueue(x);
