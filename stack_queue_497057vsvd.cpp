...
int main() {
    ds_497057vsvd::Stack<int> stk;
    for(int x:{ 73, 29, 85, 86, 83, 19, 77, 23, 7 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_497057vsvd::CQueue<int> q(20);
    for(int x:{ 73, 29, 85, 86, 83, 19, 77, 23, 7 }) q.enqueue(x);
