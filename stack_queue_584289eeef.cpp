...
int main() {
    ds_584289eeef::Stack<int> stk;
    for(int x:{ 73, 63, 92, 98, 69, 42, 43, 21, 61, 19 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_584289eeef::CQueue<int> q(20);
    for(int x:{ 73, 63, 92, 98, 69, 42, 43, 21, 61, 19 }) q.enqueue(x);
