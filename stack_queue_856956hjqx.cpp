...
int main() {
    ds_856956hjqx::Stack<int> stk;
    for(int x:{ 31, 74, 49, 60, 76, 21, 74, 41, 2, 96 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_856956hjqx::CQueue<int> q(20);
    for(int x:{ 31, 74, 49, 60, 76, 21, 74, 41, 2, 96 }) q.enqueue(x);
