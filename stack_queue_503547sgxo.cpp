...
int main() {
    ds_503547sgxo::Stack<int> stk;
    for(int x:{ 76, 35, 82, 36, 28, 98, 1, 28, 25 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_503547sgxo::CQueue<int> q(20);
    for(int x:{ 76, 35, 82, 36, 28, 98, 1, 28, 25 }) q.enqueue(x);
