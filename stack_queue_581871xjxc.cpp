...
int main() {
    ds_581871xjxc::Stack<int> stk;
    for(int x:{ 81, 53, 84, 8, 80, 8, 47, 56, 8, 66 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_581871xjxc::CQueue<int> q(20);
    for(int x:{ 81, 53, 84, 8, 80, 8, 47, 56, 8, 66 }) q.enqueue(x);
