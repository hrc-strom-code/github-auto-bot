...
int main() {
    ds_503755wnqi::Stack<int> stk;
    for(int x:{ 92, 74, 51, 47, 82, 54, 39, 58, 3, 53 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_503755wnqi::CQueue<int> q(20);
    for(int x:{ 92, 74, 51, 47, 82, 54, 39, 58, 3, 53 }) q.enqueue(x);
